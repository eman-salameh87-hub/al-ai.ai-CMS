...
(function ($) {
	...
	if ($("body").not(".is-mobile").hasClass("tt-magic-cursor")) {
		if ($(window).width() > 1024) {
			...
			let $ballHeight = 36;
			let $ballOpacity = 1; 
			let $ballBorderWidth = 2;

			let $ballMagneticWidth = 70; 
			let $ballMagneticHeight = 70; 

			let $ballAlterWidth = 100; 
			let $ballAlterHeight = 100; 

			let $ballViewWidth = 130; 
			let $ballViewHeight = 130; 

			let $ballDragWidth = 100; 
			let $ballDragHeight = 100; 

			let $ballDragMouseDownWidth = 50; 
			let $ballDragMouseDownHeight = 50; 

			let $ballArrowWidth = 100;
			let $ballArrowHeight = 100; 

			let $ballCloseWidth = 100;
			let $ballCloseHeight = 100; 
			...
			.not(".not-hide-cursor") 
			.not(".cursor-alter") 
			.not("#page-header:not(.ph-full) .tt-scroll-down-inner") 
			.not(".ph-social > ul > li a") 
			.not(".ph-share-buttons > ul > li a") 
			.not(".tt-social-buttons > ul > li a") 
